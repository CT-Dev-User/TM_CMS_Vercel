...
const CreatorManagementTable = () => {
  ...
  const fetchCreators = async () => {
    ...
    try {
      const response = await axios.get(
        "https://ot676akte0.execute-api.ap-south-1.amazonaws.com/dev/get-all-creators"
      );
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `https://ot676akte0.execute-api.ap-south-1.amazonaws.com/dev/delete-creator-by-id/${id}`
          );
          fetchCreators();
          Swal.fire("Success", "Creator data deleted successfully", "success");
        } catch (err) {
          console.error("Error deleting creator data:", err);
          Swal.fire(
            "Error",
            "Error deleting creator data. Please try again.",
            "error"
          );
        }
      }
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(
          `https://ot676akte0.execute-api.ap-south-1.amazonaws.com/dev/edit-creator/${editFormData.id}`,
          formDataToSend,
          {
        ...
        await axios.post("https://ot676akte0.execute-api.ap-south-1.amazonaws.com/dev/add-creator", formDataToSend, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      ...
      {loading ? (
        ...
        <div className="container mx-auto p-4 bg-gray-100">
          <div className="flex justify-between">
            <h2 className="text-2xl font-semibold mb-4 text-black">Creators</h2>
            <select
            ...
            <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center text-black">
              <div className="bg-white rounded-lg p-6">
                <h3 className="text-xl font-semibold mb-4">
                  {editMode ? "Edit Creator" : "Add Creator"}
                </h3>
                <div className="mb-4">
